...
// block list before accepting it. Falls back to the command_block marker
// only once the graph is exhausted with nothing found - see PLAN.md "the fix"
// for why a single-pass tier walk (the previous implementation) missed most
// of these: a candidate one tier produces can itself need substituting.
function findSubstitute(shortName, targetBlocks) {
  const seen = new Set([shortName]);
  const queue = nextCandidates(shortName);
  ...
  while (queue.length > 0) {
    ...
    queue.push(...nextCandidates(candidate));
  }
